...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthVectorOne = Math.sqrt(x1 * x1 + y1 * y1);
  const lengthVectorTwo = Math.sqrt(x2 * x2 + y2 * y2);

  return Math.acos(scalarProduct / (lengthVectorOne * lengthVectorTwo));
}
